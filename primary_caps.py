...
# --- PrimaryCapsuleTorchMLP (修改后) ---
class PrimaryCapsuleTorchMLP(nn.Module):
    """使用 MLP 直接处理输入向量的 Primary Capsules 版本 (修改后接收 input_dim)"""
    ...
    _n_transform_params = 6
    # --- 移除固定的输入维度定义 ---
    # _input_feature_dim = 256 # REMOVED
    ...
    def __init__(self,
                 ...
                 n_features: int = 0,
                 mlp_hiddens: List[int] = [128],
                 noise_scale: float = 0.0, # Default to 0.0 for safety
                 similarity_transform: bool = False,
                 activation: nn.Module = nn.SELU()
                 ):
        super().__init__()
        ...
        if n_caps_dims != self._n_transform_params:
            raise ValueError(f"n_caps_dims must be {self._n_transform_params}")

        # +++ 保存 input_dim 为实例属性 +++
        self._input_dim = input_dim # <--- SAVE input_dim
        ...
        # +++ 修改 MLP 定义以使用 input_dim +++
        self._mlp_output_dim = self._n_caps * self._n_dims
        mlp_layers = []
        # --- 使用保存的 self._input_dim 作为第一层的输入 ---
        current_mlp_dim = self._input_dim # <--- USE self._input_dim HERE
        # --- 修改结束 ---
        for hidden_dim in mlp_hiddens:
            mlp_layers.append(nn.Linear(current_mlp_dim, hidden_dim))
            mlp_layers.append(self.activation)
            current_mlp_dim = hidden_dim
        mlp_layers.append(nn.Linear(current_mlp_dim, self._mlp_output_dim))
        self.mlp_replacement = nn.Sequential(*mlp_layers)
        # +++ MLP 定义结束 +++
        print(f"PrimaryCapsuleTorchMLP initialized with input_dim={self._input_dim}")

    def forward(self, node_features: torch.Tensor) -> OutputTuple: # 参数名改为 node_features
        """
        Args:
            node_features (torch.Tensor): 输入特征，形状 [N, input_dim]。
        Returns:
            OutputTuple: 包含 pose, feature, presence, presence_logit。
        """
        # --- 在 forward 开始处添加维度检查 ---
        if node_features.dim() != 2:
             raise ValueError(f"Expected node_features shape [N, input_dim], got {node_features.shape}")
        # 检查最后一维是否匹配保存的 _input_dim
        if node_features.shape[-1] != self._input_dim:
             raise ValueError(f"Expected node_features last dim {self._input_dim}, got {node_features.shape[-1]}")
        # --- 维度检查结束 ---
        ...
        # 1. 执行 MLP 替换
        h_flat = self.mlp_replacement(node_features) # <--- 输入改变

        # 2. 执行关键的 Reshape (逻辑不变)
        try:
        ...
        # 5. 几何变换 (逻辑不变)
        if pose_params.shape[-1] != self._n_transform_params: raise ValueError(...)
        use_nonlinear_transform = True

        if geometric_transform_torch is None: raise RuntimeError(...)

        pose_transformed = geometric_transform_torch(
            pose_params, n_caps=self._n_caps, n_votes=1,
            similarity=self._similarity_transform, nonlinear=use_nonlinear_transform, as_matrix=False
        )
